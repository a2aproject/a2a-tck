...
def test_message_send_task_continuation(sut_client):
    """Test that multiple messages can be sent for the same task (task continuation)."""
    # Send first message to create a task
    first_params = {
        "message": {
            "kind": "message",
            "messageId": "test-first-continuation-message-id-" + str(uuid.uuid4()),
            "role": "user",
            "parts": [
                {"kind": "text", "text": "First message for continuation test"}
            ]
        }
    }
    ...
    # Send second message for the same task
    second_params = {
        "message": {
            "kind": "message",
            "messageId": "test-continuation-message-id-" + str(uuid.uuid4()),
            ...
            "taskId": task_id,
            "parts": [
                {"kind": "text", "text": "Second message for continuation test"}
            ]
        }
    }
    second_req = message_utils.make_json_rpc_request("message/send", params=second_params)
    second_resp = sut_client.send_json_rpc(method=second_req["method"], params=second_req["params"], id=second_req["id"])
    assert message_utils.is_json_rpc_success_response(second_resp, expected_id=second_req["id"])
    result = second_resp["result"]
